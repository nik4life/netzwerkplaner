...
export default function Home() {
  ...
  const [planAspect, setPlanAspect] = useState(1.4142);
  ...
  const surfaceSize = useMemo(() => {
    if (!stageBox.width || !stageBox.height || !planAspect) return { width: stageBox.width, height: stageBox.height };
    const availableRatio = stageBox.width / stageBox.height;
    if (availableRatio > planAspect) {
      const height = stageBox.height;
      return { width: height * planAspect, height };
    }
    const width = stageBox.width;
    return { width, height: width / planAspect };
  }, [stageBox, planAspect]);
  ...
  function avoidChannelOverlap(snap, ignoreId = null) {
    ...
    const gapOnPlan = Math.max(0.35, 1.25 * markerScale);
    const gapT = Math.min(0.2, gapOnPlan / Math.max(length, 1));
    const occupied = (floor?.markers || [])
      .filter((marker) => marker.id !== ignoreId && marker.channelId === channel.id)
      .map((marker) => typeof marker.channelT === 'number'
        ? marker.channelT
        : pointToSegment(marker, { x: channel.x1, y: channel.y1 }, { x: channel.x2, y: channel.y2 }).t);
    const free = (candidate) => occupied.every((taken) => Math.abs(taken - candidate) >= gapT);
    let t = snap.channelT;
    if (!free(t)) {
      for (let step = 1; step < 100; step += 1) {
        const delta = gapT * step;
        const candidates = [t + delta, t - delta].filter((candidate) => candidate >= 0 && candidate <= 1 && free(candidate));
        if (candidates.length) { t = candidates[0]; break; }
      }
    }
    return { ...pointOnChannel(channel, t), channelId: channel.id, channelT: t };
  }
  ...
  function handlePlanClick(event) {
    if (!floor?.plan || draggingId || mode === 'pan' || event.target.closest?.('.marker')) return;
    ...
    const finalPos = tool === 'bruestsung' ? snapToChannel(pos) : { ...pos, channelId: null, channelT: null };
    const marker = { id: uid('port'), type: tool, ports: Number(ports), x: finalPos.x, y: finalPos.y, note: '', channelId: finalPos.channelId || null, channelT: finalPos.channelT ?? null };
    patchFloor((current) => ({ ...current, markers: [...current.markers, marker] }));
    setSelectedId(marker.id);
  }

  function updateMarker(id, changes) {
    patchFloor((current) => ({ ...current, markers: current.markers.map((marker) => marker.id === id ? { ...marker, ...changes } : marker) }));
  }
  ...
  function deleteLastChannel() {
    ...
    patchFloor((current) => ({
      ...
      markers: current.markers.map((marker) => marker.channelId === removed.id ? { ...marker, channelId: null, channelT: null } : marker),
    }));
  }
  ...
  function distributeChannelPorts() {
    ...
    patchFloor((current) => {
      ...
      current.channels.forEach((channel) => {
        const ids = markers.filter((marker) => marker.channelId === channel.id).sort((a, b) => (a.channelT ?? 0) - (b.channelT ?? 0)).map((marker) => marker.id);
        if (!ids.length) return;
        markers = markers.map((marker) => {
          const index = ids.indexOf(marker.id);
          if (index < 0) return marker;
          const t = ids.length === 1 ? 0.5 : 0.025 + (index / (ids.length - 1)) * 0.95;
          return { ...marker, ...pointOnChannel(channel, t), channelT: t };
        });
      });
  ...
  function dragMarker(event, marker) {
    ...
    const pos = positionFromEvent(event);
    if (!pos) return;
    const finalPos = marker.type === 'bruestsung' ? snapToChannel(pos, marker.id) : { ...pos, channelId: null, channelT: null };
    updateMarker(marker.id, { x: finalPos.x, y: finalPos.y, channelId: finalPos.channelId || null, channelT: finalPos.channelT ?? null });
  }
  ...
  function movePan(event) {
    if (!panDrag) return;
    setPan({ x: panDrag.x + event.clientX - panDrag.startX, y: panDrag.y + event.clientY - panDrag.startY });
  }
      ...
      <section className="workspace">
        <aside className="sidebar left-sidebar">
          ...
          {mode === 'channel' ? (
            <div className="channel-help">
              <strong>Brüstungskanal</strong><span>{channelStart ? 'Endpunkt anklicken' : 'Startpunkt anklicken'}</span>
              <small>Plan, Kanal und Ports liegen jetzt auf exakt derselben PDF-Seitenfläche.</small>
            ...
            <div className="channel-help"><strong>Plan verschieben</strong><span>Klicken und ziehen</span><small>Die PDF bleibt beim Zoomen hochauflösend und kann frei verschoben werden.</small><button className="mini-button" onClick={resetView}>Ansicht zurücksetzen</button></div>
          ) : (
            <>
              <div className="tool-list">
                {Object.entries(TYPES).map(([key, item]) => <button key={key} className={tool === key ? 'tool active' : 'tool'} onClick={() => setTool(key)}><span className={`tool-icon data-tool type-${key}`}><span className="data-jack"><i></i><i></i><i></i></span></span><span><strong>{item.label}</strong><small>{item.short}</small></span></button>)}
              </div>
              <div className="form-block"><label>Ports je Punkt</label><div className="port-grid">{[1,2,4,6,8,12].map((value) => <button key={value} className={Number(ports) === value ? 'port active' : 'port'} onClick={() => setPorts(value)}>{value}</button>)}</div><input className="number-input" type="number" min="1" max="96" value={ports} onChange={(e) => setPorts(Math.max(1, Number(e.target.value) || 1))} /></div>
              {(floor?.channels || []).length > 0 && <button className="mini-button full" onClick={distributeChannelPorts}>Ports am Kanal verteilen</button>}
            </>
          )}

          <div className="form-block compact-control"><label>Symbolgröße <span>{Math.round(markerScale * 100)}%</span></label><input type="range" min="0.15" max="1" step="0.05" value={markerScale} onChange={(e) => setMarkerScale(Number(e.target.value))} /></div>
        ...
        <div className="canvas-column">
          <div className="canvas-toolbar">
            ...
            <div className="canvas-controls"><button onClick={() => setZoom((value) => Math.max(.6, +(value - .1).toFixed(2)))}>−</button><button className="zoom-label" onClick={resetView}>{Math.round(zoom * 100)}%</button><button onClick={() => setZoom((value) => Math.min(5, +(value + .1).toFixed(2)))}>＋</button><button className={mode === 'pan' ? 'pan-button active' : 'pan-button'} onClick={() => setMode(mode === 'pan' ? 'dose' : 'pan')} title="Plan verschieben">✋</button><span>{(floor?.channels || []).length} Kanäle</span><span>{stats.points} Portpunkte</span><strong>{stats.totalPorts} Ports</strong></div>
          </div>

          <div className={`plan-stage ${floor?.plan ? 'has-plan' : ''} ${mode === 'pan' ? 'pan-mode' : ''} ${panDrag ? 'panning' : ''}`} ref={stageRef} onPointerDown={startPan} onPointerMove={movePan} onPointerUp={endPan} onPointerCancel={endPan}>
            ...
            {floor?.plan && (
              <div className="plan-content stable-surface" ref={contentRef} onClick={handlePlanClick} style={{ width: `${surfaceSize.width}px`, height: `${surfaceSize.height}px`, left: '50%', top: '50%', marginLeft: `${-surfaceSize.width / 2}px`, marginTop: `${-surfaceSize.height / 2}px`, transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})` }}>
                {floor.plan.mime === 'application/pdf' ? <PdfPlan url={floor.plan.url} zoom={zoom} onAspect={setPlanAspect} /> : <img className="plan-image exact-plan-image" src={floor.plan.url} alt={floor.plan.name || 'Gebäudeplan'} draggable="false" onLoad={(event) => setPlanAspect(event.currentTarget.naturalWidth / event.currentTarget.naturalHeight)} />}
                <svg className="channel-layer" width="100%" height="100%" aria-hidden="true">{(floor.channels || []).map((channel) => <line key={channel.id} x1={`${channel.x1}%`} y1={`${channel.y1}%`} x2={`${channel.x2}%`} y2={`${channel.y2}%`} />)}{channelStart && <circle cx={`${channelStart.x}%`} cy={`${channelStart.y}%`} r="5" />}</svg>
                {(floor?.markers || []).map((marker) => <button key={marker.id} className={`marker port-marker type-${marker.type} ${selectedId === marker.id ? 'selected' : ''} ${marker.channelId ? 'snapped' : ''}`} style={{ left: `${marker.x}%`, top: `${marker.y}%`, '--marker-scale': markerScale }} onPointerDown={(event) => startMarkerDrag(event, marker)} onPointerMove={(event) => dragMarker(event, marker)} onPointerUp={endMarkerDrag} onPointerCancel={endMarkerDrag} onClick={(event) => { event.stopPropagation(); setSelectedId(marker.id); }} title={`${TYPES[marker.type]?.label || 'Datenport'} · ${marker.ports} Ports`}><span className="cad-port"><i></i><i></i><i></i></span>{Number(marker.ports) > 1 && <b>{marker.ports}</b>}</button>)}
              </div>
            )}
          </div>
        </div>
        ...
        <aside className="sidebar right-sidebar">
          <h2>Auswertung</h2><div className="big-stat"><strong>{stats.totalPorts}</strong><span>Ports in {floor?.name}</span></div>
          <div className="stats-list">{Object.entries(TYPES).map(([key, type]) => <div key={key} className="stat-row"><span><i className={`stat-dot type-${key}`}></i>{type.label}</span><strong>{stats.byType[key].ports}</strong><small>{stats.byType[key].points} Portpunkte</small></div>)}</div>
          <div className="building-total"><span>Gebäude gesamt</span><strong>{data.floors.reduce((sum, item) => sum + (item.markers || []).reduce((part, marker) => part + Number(marker.ports || 0), 0), 0)} Ports</strong></div>
          <div className="selection-panel"><h3>Ausgewählter Portpunkt</h3>{!selected && <p className="hint">Einen Portpunkt anklicken, um ihn zu bearbeiten.</p>}{selected && <><label>Montageart<select value={selected.type} onChange={(e) => updateMarker(selected.id, { type: e.target.value, channelId: e.target.value === 'bruestsung' ? selected.channelId : null, channelT: e.target.value === 'bruestsung' ? selected.channelT : null })}>{Object.entries(TYPES).map(([key,item]) => <option key={key} value={key}>{item.label}</option>)}</select></label><label>Ports<input type="number" min="1" value={selected.ports} onChange={(e) => updateMarker(selected.id, { ports: Math.max(1, Number(e.target.value) || 1) })} /></label><label>Notiz<textarea rows="3" placeholder="z. B. Arbeitsplatz / Drucker / Sonderbedarf" value={selected.note || ''} onChange={(e) => updateMarker(selected.id, { note: e.target.value })} /></label>{selected.channelId && <div className="snap-info">✓ Am Brüstungskanal eingerastet</div>}<button className="button danger" onClick={() => deleteMarker(selected.id)}>Portpunkt löschen</button></>}</div>
        </aside>
      </section>
    </main>
  );
